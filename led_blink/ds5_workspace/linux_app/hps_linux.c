...
void munmap_hps_peripherals() {
    if (munmap(hps_gpio, hps_gpio_span) != 0) {
        printf("Error: fpga_leds munmap() failed\n");
        printf("    errno = %s\n", strerror(errno));
        close(fd_dev_mem);
        exit(EXIT_FAILURE);
    }
...
void mmap_fpga_nonlw_peripherals() {
    fpga_leds = mmap(NULL, 2*fpga_leds_span, PROT_READ | PROT_WRITE, MAP_SHARED, fd_dev_mem, fpga_leds_ofst-0x40);
    if (fpga_leds == MAP_FAILED) {
        printf("Error: fpga leds mmap() failed.\n");
        printf("    errno = %s\n", strerror(errno));
        close(fd_dev_mem);
        exit(EXIT_FAILURE);
    }
...
void munmap_fpga_nonlw_peripherals() {
    if (munmap(fpga_leds, fpga_leds_span) != 0) {
...
void mmap_fpga_peripherals() {
    ...
    fpga_leds = h2f_lw_axi_master + PIO_0_BASE; //HPS_FPGA_LEDS_BASE;
}
...
void mmap_peripherals() {
    ...
    //mmap_fpga_nonlw_peripherals();
}

void munmap_peripherals() {
    munmap_hps_peripherals();
    munmap_fpga_peripherals();
    //munmap_fpga_nonlw_peripherals();
}
...
void handle_hps_led() {
    ...
    bool toggle_hps_led = 1;//(~hps_gpio_input);// & HPS_KEY_N_MASK);
...
void handle_fpga_leds() {
    ...
    if (leds_mask != (0x01 << (PIO_0_DATA_WIDTH - 1))) {
        // rotate leds
        leds_mask <<= 1;
    } else {
        // reset leds
...
int main() {
    ...
    printf("Opened physical memory device\n\n");
    mmap_peripherals();
    printf("memory map peripherals\n\n");
    setup_hps_gpio();
    printf("addr: %x\n\n", fpga_leds);
    //setup_fpga_leds();
    //size_t val = alt_read_word(fpga_leds);
    //printf("val: %x\n", val);

    /*for(i = 0; i < 0x80; i++) {
    	alt_write_word(fpga_leds+i, 0xFFFFFFFFFFFFFFFF);
    }
    //	alt_write_word(fpga_leds,0xAAAAAAAAAAAAAAAA);
    size_t dir = alt_read_word(fpga_leds+0x4);
    size_t val = alt_read_word(fpga_leds);
    printf("dir: %x\n", dir);
    printf("val: %x\n", val);
    printf("here 0\n\n");*/
