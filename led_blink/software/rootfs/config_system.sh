...
# kernel boots. We had previously cocnfigured U-Boot to supply the command-line
# argument "console=ttyS0,115200" to the linux kerne. This argument instructs
# the kernel to use serial console "ttyS0" as the boot shell, so here we choose
# to use the same seriall console for the login shell
...
# Create a user and a passowrd. In this example, we create a user called
# "qubits" with password "1234". Note that we compute an encrypted version of
...
# Ubuntu rquires the admin to be part of the "adm" and "sudo" groups, so add
# the previously cireated user to the 2 groups
